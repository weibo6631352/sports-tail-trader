...
**默认不写测试。** 现有代码已经过实盘验证，单元测试维护成本高且常常落后于真实行为，统一删除 ``tests/`` 目录。

- 写代码时**只为本次新写的、未经过任何实盘检验的关键逻辑**写局部测试，验证完毕即可删除。
- 不补测试覆盖率，不在 PR 里要求"补回归测试"。
- 改完跑 ``ruff check`` + 编译 + 启动一遍后端，能跑通就算通过；行为对错以实盘观察 + audit_events 复盘为准。
- 历史"必须补测试"的强制项（自动下单 / 风控 / 资金计算 / 订单幂等 / 恢复 / reconcile）已不再要求——这些路径全部经过实盘洗礼，再加测试只会拖慢迭代。
