...
原 ``strategies.current.trading.hooks`` 的 size_entry / decide_entry /
...
from strategies.current.trading.allocation import (
...
from strategies.current.trading.gates import _ask_depth_notional
from strategies.current.trading.helpers import _metadata_text
from strategies.current.trading.risk_limits import _apply_tail_risk_limits
...
class QuantDecider:
    ...
    def _decide_position_action(self, context: ExtensionContext) -> ExtensionDecision:
        from strategies.current.position_plan import exit_price_for_context
