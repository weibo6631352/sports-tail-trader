"""体育盘口数学概率评估(归一化 probability + 详细审计)。

每类盘口的"剩余比赛能否让我方持仓输/赢"概率算法不同(MLB Totals 用单半局得分
泊松分布、Soccer HT 用伤停时间、NBA ML 用剩余分钟+分差等),本模块用统一
``MathProbResult`` 接口让上层（quant_decider / quant_signal）拿到归一化
[0,1] 锁定度 + 关键变量,无需各自实现锁定算法。

设计原则:
- ``probability=1.0`` = 数学上 100% 锁定(剩余赛果无法影响持仓胜负)
- ``probability=0.0`` = 完全不锁定 / 不支持的盘口类型
- 中间值反映真实剩余胜率(基于历史 base rate);workflow 可按阈值取舍
...
from polymarket_trader.sports.types import (
    LiveGameState,
    LiveGameStatus,
    SportsMarketSide,
    SportsMarketType,
...
class MathProbResult:
    """数学概率评估结果,归一化 + 可审计。"""

    probability: Decimal  # [0, 1],1=full lock
...
def _mlb_totals_lock(
    ...
    """MLB Totals U/O 数学概率: 剩余 half-innings 单独得分独立 Poisson 近似。"""

    state = game.baseball_state
    if state is None or state.current_inning is None:
        return MathProbResult(_ZERO, "mlb_totals", "missing_baseball_state", {})
...
def _baseball_moneyline_lock(
    ...
    """Baseball Moneyline 数学概率: 基于 lead × 剩余半局 normal-approx reversal 概率。"""

    state = game.baseball_state
    if state is None or state.current_inning is None:
        return MathProbResult(_ZERO, "baseball_ml", "missing_baseball_state", {})
    home = int(game.home_score or 0)
    away = int(game.away_score or 0)
    ...
    if side == SportsMarketSide.HOME:
        ...
        return MathProbResult(_ZERO, "baseball_ml", "unsupported_side", {})

    if lead <= 0:
        # 我方落后或平局,不锁定(实际胜率 < 50%,不用 math_prob 作进场信号)
        return MathProbResult(
            _ZERO,
            "baseball_ml",
            "side_not_leading",
...
def _soccer_halftime_lock(
    ...
    if state is None:
        return MathProbResult(_ZERO, "soccer_halftime", "missing_soccer_state", {})
    ...
    if period != "first_half":
        return MathProbResult(_ZERO, "soccer_halftime", "unknown_period", {"period": period})
...
def _estimate_remaining_seconds(game: LiveGameState) -> int | None:
    ...
    用经验时长估算让 math_prob 仍可基于"剩余时间"算 reversal 概率，而不是因为
...
def _basketball_ml_lock(side: SportsMarketSide, game: LiveGameState) -> MathProbResult:
    """NBA/WNBA ML：lead × 剩余秒数双边正态近似 reversal。"""
    if side not in {SportsMarketSide.HOME, SportsMarketSide.AWAY}:
        return MathProbResult(_ZERO, "basketball_ml", "unsupported_side", {})
    lead = game.score_diff_for(side)
    if lead <= 0:
        return MathProbResult(_ZERO, "basketball_ml", "side_not_leading", {"lead": lead})
    remaining = _estimate_remaining_seconds(game)
    if remaining is None:
        return MathProbResult(_ZERO, "basketball_ml", "missing_remaining_time", {"lead": lead})
    if remaining <= 0:
        return MathProbResult(_ONE, "basketball_ml", "no_remaining_time", {"lead": lead})
...
def _basketball_spreads_lock(
    ...
    if side not in {SportsMarketSide.HOME, SportsMarketSide.AWAY}:
        return MathProbResult(_ZERO, "basketball_spreads", "unsupported_side", {})
    ...
    if adjusted <= 0:
        return MathProbResult(_ZERO, "basketball_spreads", "not_leading_after_handicap",
                              {"adjusted_lead": adjusted})
    var_diff = 2.0 * _BASKETBALL_VAR_PER_SECOND * remaining
...
def _soccer_ml_lock(side: SportsMarketSide, game: LiveGameState) -> MathProbResult:
    ...
    """
    if side not in {SportsMarketSide.HOME, SportsMarketSide.AWAY}:
        return MathProbResult(_ZERO, "soccer_ml", "unsupported_side", {})
    ...
    if remaining is None or remaining <= 0:
        if side == SportsMarketSide.HOME:
            ...
            won = away > home
        return MathProbResult(_ONE if won else _ZERO, "soccer_ml", "no_remaining_time",
                              {"home": home, "away": away})
    # 剩余进球期望 = 单边率 × 剩余秒
    ...
    if sigma <= 0.01:
        # 时间太短，按当前比分判定
        if side == SportsMarketSide.HOME:
            return MathProbResult(_ONE if home > away else _ZERO, "soccer_ml",
                                  "negligible_remaining", {"home": home, "away": away})
        if side == SportsMarketSide.AWAY:
            return MathProbResult(_ONE if away > home else _ZERO, "soccer_ml",
                                  "negligible_remaining", {"home": home, "away": away})
        return MathProbResult(_ONE if home == away else _ZERO, "soccer_ml",
                              "negligible_remaining", {"home": home, "away": away})
    lead = home - away
    if side == SportsMarketSide.HOME:
        if lead <= 0:
            return MathProbResult(_ZERO, "soccer_ml", "not_leading", {"lead": lead})
...
def _soccer_totals_lock(
    ...
    if side not in {SportsMarketSide.OVER, SportsMarketSide.UNDER}:
        return MathProbResult(_ZERO, "soccer_totals", "unsupported_side", {})
    total = Decimal(int(game.total_score))
    remaining = _estimate_remaining_seconds(game)
    if remaining is None or remaining <= 0:
        won = (total > line) if side == SportsMarketSide.OVER else (total < line)
        return MathProbResult(_ONE if won else _ZERO, "soccer_totals", "game_ended",
                              {"total": str(total), "line": str(line)})
    if side == SportsMarketSide.OVER and total > line:
        return MathProbResult(_ONE, "soccer_totals", "already_over_win", {})
    if side == SportsMarketSide.UNDER and total >= line:
        return MathProbResult(_ZERO, "soccer_totals", "already_over_lose", {})
...
def _soccer_spreads_lock(
    ...
    if side not in {SportsMarketSide.HOME, SportsMarketSide.AWAY}:
        return MathProbResult(_ZERO, "soccer_spreads", "unsupported_side", {})
    raw_lead = game.score_diff_for(side)
    adjusted = float(raw_lead) + (float(line) if side == SportsMarketSide.HOME else -float(line))
    remaining = _estimate_remaining_seconds(game)
    if remaining is None or remaining <= 0:
        return MathProbResult(_ONE if adjusted > 0 else _ZERO, "soccer_spreads",
                              "no_remaining_time", {"adjusted_lead": adjusted})
    if adjusted <= 0:
        return MathProbResult(_ZERO, "soccer_spreads", "not_leading_after_handicap",
                              {"adjusted_lead": adjusted})
    lam = _SOCCER_GOAL_RATE_PER_SEC * remaining
    sigma = math.sqrt(2.0 * lam)
    if sigma <= 0.01:
        return MathProbResult(_ONE, "soccer_spreads", "negligible_remaining",
                              {"adjusted_lead": adjusted})
    z = adjusted / sigma
    p_reversal = 0.5 * math.erfc(z / math.sqrt(2.0))
    lock = max(0.0, min(1.0, 1.0 - p_reversal))
...
def _tennis_ml_lock(side: SportsMarketSide, game: LiveGameState) -> MathProbResult:
    ...
    if state is None:
        return MathProbResult(_ZERO, "tennis_ml", "missing_tennis_state", {})
    if side not in {SportsMarketSide.HOME, SportsMarketSide.AWAY}:
        return MathProbResult(_ZERO, "tennis_ml", "unsupported_side", {})
    ...
    if my_sets is None or opp_sets is None:
        return MathProbResult(_ZERO, "tennis_ml", "missing_sets", {})
    best_of = state.best_of or 3
    need_sets = (best_of // 2) + 1
    if my_sets >= need_sets:
        return MathProbResult(_ONE, "tennis_ml", "match_already_won",
                              {"my_sets": my_sets, "opp_sets": opp_sets, "best_of": best_of})
    if opp_sets >= need_sets:
        return MathProbResult(_ZERO, "tennis_ml", "match_already_lost",
                              {"my_sets": my_sets, "opp_sets": opp_sets, "best_of": best_of})
    ...
    if opp_need > remaining_sets:
        return MathProbResult(_ONE, "tennis_ml", "opp_cannot_win",
                              {"my_sets": my_sets, "opp_sets": opp_sets, "best_of": best_of})
...
def _tennis_totals_lock(
    ...
    state = game.tennis_state
    if state is None:
        return MathProbResult(_ZERO, "tennis_totals", "missing_tennis_state", {})
    if side not in {SportsMarketSide.OVER, SportsMarketSide.UNDER}:
        return MathProbResult(_ZERO, "tennis_totals", "unsupported_side", {})
    ...
    if max_remaining_sets <= 0:
        ...
        return MathProbResult(_ONE if won else _ZERO, "tennis_totals", "match_ended",
    ...
    if sigma <= 0.01:
        won = (games_played > line) if side == SportsMarketSide.OVER else (games_played < line)
        return MathProbResult(_ONE if won else _ZERO, "tennis_totals", "negligible_remaining",
                              {"games_played": games_played, "line": str(line)})
...
def _hockey_ml_lock(side: SportsMarketSide, game: LiveGameState) -> MathProbResult:
    """Hockey ML：Skellam 双边剩余进球差。"""
    if side not in {SportsMarketSide.HOME, SportsMarketSide.AWAY}:
        return MathProbResult(_ZERO, "hockey_ml", "unsupported_side", {})
    lead = game.score_diff_for(side)
    if lead <= 0:
        return MathProbResult(_ZERO, "hockey_ml", "not_leading", {"lead": lead})
    remaining = _estimate_remaining_seconds(game)
    if remaining is None or remaining <= 0:
        return MathProbResult(_ONE, "hockey_ml", "no_remaining_time", {"lead": lead})
    lam = _HOCKEY_GOAL_RATE_PER_SEC * remaining
    sigma = math.sqrt(2.0 * lam)
    if sigma <= 0.01:
        return MathProbResult(_ONE, "hockey_ml", "negligible_remaining", {"lead": lead})
    z = lead / sigma
    p_reversal = 0.5 * math.erfc(z / math.sqrt(2.0))
    lock = max(0.0, min(1.0, 1.0 - p_reversal))
    return MathProbResult(
...
def _soccer_btts_lock(side: SportsMarketSide, game: LiveGameState) -> MathProbResult:
    ...
    if side not in {SportsMarketSide.YES, SportsMarketSide.NO}:
        return MathProbResult(_ZERO, "soccer_btts", "unsupported_side", {})
    h = game.home_score
    a = game.away_score
    both_scored = h >= 1 and a >= 1
    remaining = _estimate_remaining_seconds(game)
    if remaining is None:
        return MathProbResult(_ZERO, "soccer_btts", "missing_remaining_time", {})
...
def _soccer_draw_ft_lock(side: SportsMarketSide, game: LiveGameState) -> MathProbResult:
    """Soccer Draw Yes/No (整场是否平局) — 用 Skellam(0) 算剩余进球差 = 0 概率。"""
    if side not in {SportsMarketSide.YES, SportsMarketSide.NO}:
        return MathProbResult(_ZERO, "soccer_draw_ft", "unsupported_side", {})
    h = game.home_score
    a = game.away_score
    remaining = _estimate_remaining_seconds(game)
    if remaining is None:
        return MathProbResult(_ZERO, "soccer_draw_ft", "missing_remaining_time", {})
    if remaining <= 0 or game.status == LiveGameStatus.ENDED:
        is_draw = h == a
...
def _baseball_nrfi_lock(side: SportsMarketSide, game: LiveGameState) -> MathProbResult:
    ...
    """
    if side not in {SportsMarketSide.YES, SportsMarketSide.NO}:
        return MathProbResult(_ZERO, "baseball_nrfi", "unsupported_side", {})
    state = game.baseball_state
    if state is None or state.current_inning is None:
        return MathProbResult(_ZERO, "baseball_nrfi", "missing_baseball_state", {})
    ...
    if inning >= 2:
        ...
        if first_h_runs is None or first_a_runs is None:
            # 数据缺：保守按 unsupported
            return MathProbResult(_ZERO, "baseball_nrfi", "missing_first_inning_runs", {})
...
def _hockey_spreads_lock(
    ...
    if side not in {SportsMarketSide.HOME, SportsMarketSide.AWAY}:
        return MathProbResult(_ZERO, "hockey_spreads", "unsupported_side", {})
    raw_lead = game.score_diff_for(side)
    adjusted = float(raw_lead) + (float(line) if side == SportsMarketSide.HOME else -float(line))
    remaining = _estimate_remaining_seconds(game)
    if remaining is None:
        return MathProbResult(_ZERO, "hockey_spreads", "missing_remaining_time", {})
    if remaining <= 0:
        return MathProbResult(
    ...
    if adjusted <= 0:
        return MathProbResult(_ZERO, "hockey_spreads", "not_leading_after_handicap",
                              {"adjusted_lead": adjusted, "raw_lead": raw_lead, "line": str(line)})
    lam = _HOCKEY_GOAL_RATE_PER_SEC * remaining
    sigma = math.sqrt(2.0 * lam)
    if sigma <= 0.01:
        return MathProbResult(_ONE, "hockey_spreads", "negligible_remaining", {"adjusted_lead": adjusted})
    z = adjusted / sigma
    p_reversal = 0.5 * math.erfc(z / math.sqrt(2.0))
    lock = max(0.0, min(1.0, 1.0 - p_reversal))
    return MathProbResult(
...
def _tennis_set_winner_lock(side: SportsMarketSide, game: LiveGameState) -> MathProbResult:
    ...
    state = game.tennis_state
    if state is None:
        return MathProbResult(_ZERO, "tennis_set_winner", "missing_tennis_state", {})
    if side not in {SportsMarketSide.HOME, SportsMarketSide.AWAY}:
        return MathProbResult(_ZERO, "tennis_set_winner", "unsupported_side", {})
    ...
    if my_games is None or opp_games is None:
        return MathProbResult(_ZERO, "tennis_set_winner", "missing_set_games", {})
    # Set 先到 6 局且领先 2+，否则 7-5 / 7-6 (tiebreak)
    if my_games >= 6 and my_games >= opp_games + 2:
        return MathProbResult(_ONE, "tennis_set_winner", "set_already_won",
                              {"my_games": my_games, "opp_games": opp_games})
    if opp_games >= 6 and opp_games >= my_games + 2:
        return MathProbResult(_ZERO, "tennis_set_winner", "set_already_lost",
                              {"my_games": my_games, "opp_games": opp_games})
...
def _nfl_ml_lock(side: SportsMarketSide, game: LiveGameState) -> MathProbResult:
    """NFL ML：lead × 剩余秒数双边正态近似 reversal。"""
    if side not in {SportsMarketSide.HOME, SportsMarketSide.AWAY}:
        return MathProbResult(_ZERO, "nfl_ml", "unsupported_side", {})
    lead = game.score_diff_for(side)
    if lead <= 0:
        return MathProbResult(_ZERO, "nfl_ml", "not_leading", {"lead": lead})
    remaining = _estimate_remaining_seconds(game)
    if remaining is None:
        return MathProbResult(_ZERO, "nfl_ml", "missing_remaining_time", {})
    if remaining <= 0:
        return MathProbResult(_ONE, "nfl_ml", "no_remaining_time", {"lead": lead})
...
def _nfl_totals_lock(side: SportsMarketSide, line: Decimal, game: LiveGameState) -> MathProbResult:
    """NFL Totals：双边总得分正态近似。"""
    if side not in {SportsMarketSide.OVER, SportsMarketSide.UNDER}:
        return MathProbResult(_ZERO, "nfl_totals", "unsupported_side", {})
    total = Decimal(int(game.total_score))
    remaining = _estimate_remaining_seconds(game)
    if remaining is None:
        return MathProbResult(_ZERO, "nfl_totals", "missing_remaining_time", {})
    if remaining <= 0:
        won = (total > line) if side == SportsMarketSide.OVER else (total < line)
        return MathProbResult(_ONE if won else _ZERO, "nfl_totals", "game_ended",
                              {"total": str(total), "line": str(line)})
    if side == SportsMarketSide.OVER and total > line:
        return MathProbResult(_ONE, "nfl_totals", "already_over_win", {})
    if side == SportsMarketSide.UNDER and total >= line:
        return MathProbResult(_ZERO, "nfl_totals", "already_over_lose", {})
...
def _nfl_spreads_lock(side: SportsMarketSide, line: Decimal, game: LiveGameState) -> MathProbResult:
    """NFL Spreads：让分后净 lead 正态近似。"""
    if side not in {SportsMarketSide.HOME, SportsMarketSide.AWAY}:
        return MathProbResult(_ZERO, "nfl_spreads", "unsupported_side", {})
    raw_lead = game.score_diff_for(side)
    adjusted = float(raw_lead) + (float(line) if side == SportsMarketSide.HOME else -float(line))
    remaining = _estimate_remaining_seconds(game)
    if remaining is None:
        return MathProbResult(_ZERO, "nfl_spreads", "missing_remaining_time", {})
    if remaining <= 0:
        return MathProbResult(_ONE if adjusted > 0 else _ZERO, "nfl_spreads", "no_remaining_time",
                              {"adjusted_lead": adjusted})
    if adjusted <= 0:
        return MathProbResult(_ZERO, "nfl_spreads", "not_leading_after_handicap",
                              {"adjusted_lead": adjusted})
    var_diff = 2.0 * _NFL_VAR_PER_SECOND * remaining
...
def _cricket_chase_lock(side: SportsMarketSide, game: LiveGameState) -> MathProbResult:
    ...
    if state is None or state.target is None:
        return MathProbResult(_ZERO, "cricket_ml", "missing_cricket_target", {})
    if side not in {SportsMarketSide.HOME, SportsMarketSide.AWAY}:
        return MathProbResult(_ZERO, "cricket_ml", "unsupported_side", {})
...
def _hockey_totals_lock(
    ...
    if side not in {SportsMarketSide.OVER, SportsMarketSide.UNDER}:
        return MathProbResult(_ZERO, "hockey_totals", "unsupported_side", {})
    total = Decimal(int(game.total_score))
    remaining = _estimate_remaining_seconds(game)
    if remaining is None or remaining <= 0:
        won = (total > line) if side == SportsMarketSide.OVER else (total < line)
        return MathProbResult(_ONE if won else _ZERO, "hockey_totals", "game_ended",
                              {"total": str(total), "line": str(line)})
    if side == SportsMarketSide.OVER and total > line:
        return MathProbResult(_ONE, "hockey_totals", "already_over_win", {})
    if side == SportsMarketSide.UNDER and total >= line:
        return MathProbResult(_ZERO, "hockey_totals", "already_over_lose", {})
...
# 子段盘口，必须有专用公式才能跑 math_prob，否则返回 sub_scope_no_dedicated_lock。
_SUB_SCOPE_TOKENS = (
...
def _is_sub_scope_without_dedicated_lock(
    ...
    """slug 是子段 scope 但当前没有专用 math_prob 公式 → True（视为 unsupported）。
...
def evaluate_math_prob(
    ...
    *,
    ...
    """统一数学概率评估。按 (market_type, sport) 分派；通用兜底覆盖所有盘口。

    优先级：sport-specific 专属公式 > generic_*_lock 兜底。
    无 sport 信息或 variance 表无该 sport → 返回 unsupported（probability=0），
    workflow 可 fallback 到 goalserve_implied / microprice。
    """
    ...
    # first-set 之类已结束子段会算出完全错误的 lock_prob，导致 entry math_prob veto
    # 错杀已锁定赢方（或漏放过已锁定输方）。
    ...
    # 专用 math_prob 公式 → 在分派前先返回 UNSUPPORTED，让上层 evaluator 自己
    # 处理（顶层 _math_prob_veto 对 unsupported 放行，§17 不放过可盈利市场）。
    slug_lc = (market_slug or "").lower()
    if _is_sub_scope_without_dedicated_lock(slug_lc, market_type, sport, game):
        return MathProbResult(_ZERO, "sub_scope", "sub_scope_no_dedicated_lock", {"slug": market_slug})
