...
def pipeline_health_summary(
    *,
    cpu_samples_map: dict,
    step_samples_map: dict,
    worker_ticks: dict,
    coroutine_exceptions: dict,
    window_seconds: float = 60.0,
) -> list[dict]:
    """综合健康视图: pipeline CPU + steps + worker tick + 异常 + 健康判断.
    ...
    按 share_of_total_cpu_pct 倒序.
    """
    # 复用 cpu summary 拿到 share_pct + cpu stats
    cpu_rows = pipeline_cpu_summary(cpu_samples_map, window_seconds=window_seconds)
    cpu_by_name = {r["name"]: r for r in cpu_rows}
    # step 聚合
    step_by_pipeline = pipeline_step_summary(step_samples_map, window_seconds=window_seconds)
    # 把 worker_ticks 也按 pipeline 名 fuzzy match (worker 名常含 pipeline 关键字)
    worker_by_name: dict[str, dict] = {}
    for w_name, stat in worker_ticks.items():
        intervals = list(stat.get("intervals", []))
        expected = stat.get("expected_interval_s")
        actual_avg = sum(intervals) / len(intervals) if intervals else None
        drift_pct = None
        if expected and actual_avg and expected > 0:
            drift_pct = (actual_avg - expected) / expected * 100
        worker_by_name[w_name] = {
            "tick_count": stat.get("count", 0),
            "expected_interval_s": expected,
            "actual_avg_interval_s": round(actual_avg, 3) if actual_avg is not None else None,
            "drift_pct": round(drift_pct, 1) if drift_pct is not None else None,
            "errors": stat.get("errors", 0),
        }

    # 合并: 收集所有 pipeline 名 (cpu_track 接入的 + worker_ticks 中独立的)
    all_pipelines = set(cpu_by_name.keys()) | set(worker_by_name.keys())
    result: list[dict] = []
    for pipeline in all_pipelines:
        cpu_row = cpu_by_name.get(pipeline, {})
        worker_row = worker_by_name.get(pipeline)
        steps = step_by_pipeline.get(pipeline, [])
        exc_count = coroutine_exceptions.get(pipeline, 0)

        calls = cpu_row.get("calls", 0)
        cpu_share = cpu_row.get("share_of_total_cpu_pct", 0.0)
        drift = worker_row.get("drift_pct") if worker_row else None
        error_rate = (exc_count / calls * 100) if calls > 0 else 0.0

        # 健康判断
        reasons = []
        if calls == 0 and (worker_row is None or worker_row.get("tick_count", 0) == 0):
            health = "idle"
        else:
            if error_rate > 10:
                reasons.append(f"error_rate={error_rate:.1f}%")
            elif error_rate > 1:
                reasons.append(f"error_rate={error_rate:.1f}%")
            if cpu_share > 70:
                reasons.append(f"cpu_share={cpu_share:.1f}%")
            elif cpu_share > 30:
                reasons.append(f"cpu_share={cpu_share:.1f}%")
            if drift is not None:
                if drift > 100:
                    reasons.append(f"drift={drift:.0f}%")
                elif drift > 30:
                    reasons.append(f"drift={drift:.0f}%")
            # 定级 (取最严)
            if error_rate > 10 or cpu_share > 70 or (drift is not None and drift > 100):
                health = "unhealthy"
            elif error_rate > 1 or cpu_share > 30 or (drift is not None and drift > 30):
                health = "degraded"
            else:
                health = "healthy"
        ...
        result.append({
            ...
            "calls_total": calls,
            "calls_per_second": cpu_row.get("calls_per_second", 0.0),
            "cpu_avg_ms": cpu_row.get("cpu_avg_ms"),
            "cpu_p99_ms": cpu_row.get("cpu_p99_ms"),
            "wall_avg_ms": cpu_row.get("wall_avg_ms"),
            "cpu_efficiency": cpu_row.get("cpu_efficiency"),
            "share_of_total_cpu_pct": cpu_share,
            "error_count": exc_count,
            "error_rate_pct": round(error_rate, 2),
            "worker_tick": worker_row,
            "steps": steps,
        })
    result.sort(key=lambda r: r.get("share_of_total_cpu_pct", 0.0), reverse=True)
    return result
