...
def run_entry_replay(
    ...
    *,
    ...
    plan = TradingDecisionService(
        ...
        kelly_fraction=_optional_decimal(budgets, "kelly_fraction") or Decimal("0.25"),
        kelly_max_position_fraction=_optional_decimal(budgets, "kelly_max_position_fraction") or Decimal("0.10"),
        kelly_min_edge=_optional_decimal(budgets, "kelly_min_edge") or Decimal("0.02"),
...
def _serialize_plan(plan: EntryPlan) -> dict[str, Any]:
    return {
        ...
        if plan.intent is None
            ...
            "amount_usdc": None if plan.intent.amount_usdc is None else str(plan.intent.amount_usdc),
            "size_shares": None if plan.intent.size_shares is None else str(plan.intent.size_shares),
