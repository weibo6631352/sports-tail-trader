...
class MarketTickWorker:
    ...
    def __init__(
        ...
        *,
        ...
        self._decision_builder = decision_context_builder
        self._order_gateway = order_gateway or OrderGateway()
        ...
        self._order_result_processor = TradingOrderResultProcessor(
            host=self,
            decision_context_builder=self._decision_builder,
    ...
    async def _handle_orderbook_snapshot_updated(
        ...
        # 合成事件 dedupe：position_heartbeat 每 5s 发合成 event 兜底薄盘
        ...
        if has_snapshot and has_cid and has_tid:
            ...
            if position is not None and position.shares > Decimal("0"):
                ...
                orderbook = self._decision_builder.lookup_orderbook(event.token_id)
    ...
    async def _execute_position_exit_if_needed(
        ...
        *,
        ...
        market = self._decision_builder.resolve_market(
        ...
        quant_decision = self._decision_builder.quant_decide(
            DecisionContext(
                ...
                market=market,
                token_id=position.token_id,
                orderbook=self._decision_builder.lookup_orderbook(position.token_id),
    ...
    def _market_fromsnapshot_position(
        ...
        if position is None:
            return None
        market = self._decision_builder.resolve_market(condition_id=condition_id, token_id=token_id)
        return market
    ...
    async def _execute_managed_intent(
        ...
        *,
        ...
        if isinstance(intent, CancelOrderIntent):
            return await self._order_gateway.cancel(intent)
        if isinstance(intent, ReplaceOrderIntent):
            result = await self._order_gateway.replace(intent)
            logger.info(
            ...
            return result
        return await self._order_gateway.review_intent(
            intent,
            market=market,
