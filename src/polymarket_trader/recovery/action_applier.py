...
from polymarket_trader.recovery.reconcile_service import ReconcileAction, ReconcileActionType
...
class ReconcileActionApplier:
    ...
    async def _apply_cancel(self, action: ReconcileAction, account_snapshot: AccountSnapshot) -> None:
        ...
        review = await self._order_gateway.cancel(cancel_intent)
    ...
    async def _apply_replace(
        ...
        review = await self._order_gateway.replace(replace_intent)
        result = review.order_result
